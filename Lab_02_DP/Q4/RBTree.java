...
public class RBTree<T extends Comparable<T>> {
	...
	public void prop3(Node<T> node, Set<Integer> height, int cur_height){
		if (node.value == null){
			cur_height += 1;
			height.add(cur_height);
		}else{
			if (node.colour == Colour.BLACK){
				cur_height += 1;
			}
			prop3(node.left, height,cur_height);
			prop3(node.right, height,cur_height);
		}
	}
	...
	public ArrayList<Node<T>> findLeaf(Node<T> node){
		...
		if(node!=null){
			leafNode.addAll(findLeaf(node.left));
			if (node.value == null){
				leafNode.add(node);
			}
			leafNode.addAll(findLeaf(node.right));
		}
		return leafNode;
	}
